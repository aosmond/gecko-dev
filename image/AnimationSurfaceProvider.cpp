...
{
  // Because we now only decode as the animation display progresses, and because
  // we may be continuously decoding if the required memory footprint is too
  // high, we now run into the case where we still have a decoder when we free
  // the surface provider. Before we would simply continue decoding until it was
  // done, free mDecoder on the decoder threads, and then free the surface
  // provider. The new beahviour trips the deadlock detector because we invert
  // our typical order of mutex acquisition between SurfaceCache and
  // SourceBuffer. Simplist thing to do is to free off this execution context.
  if (mDecoder) {
    NS_ReleaseOnMainThreadSystemGroup(mDecoder.forget(), /* aAlwaysProxy = */ true);
  }

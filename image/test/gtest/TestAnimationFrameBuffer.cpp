...
static void
PrepareForDiscardingQueue(AnimationFrameRetainedQueue& aQueue)
{
                       ...
                       AnimationFrameBuffer::InsertStatus aExpectedStatus)
{
  ...
  EXPECT_TRUE(aQueue.IsLastFrame(frame));
            ...
            const imgFrame* aFirstFrame)
{
  ...
  if (!aQueue.MayDiscard()) {
    const AnimationFrameRetainedQueue& queue =
      *static_cast<AnimationFrameRetainedQueue*>(&aQueue);
...
TEST_F(ImageAnimationFrameBuffer, RetainedInitialState)
{
  ...
  AnimationFrameRetainedQueue buffer(kThreshold, kBatch, 0);
...
TEST_F(ImageAnimationFrameBuffer, StartAfterBeginning)
{
  ...
  AnimationFrameRetainedQueue buffer(kThreshold, kBatch, kStartFrame);
...
TEST_F(ImageAnimationFrameBuffer, DiscardingLoop)
{
  ...
  AnimationFrameRetainedQueue retained(kThreshold, kBatch, kStartFrame);

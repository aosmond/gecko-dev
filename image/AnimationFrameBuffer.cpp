...
AnimationFrameRetainedQueue::AnimationFrameRetainedQueue(size_t aThreshold,
                                                         size_t aBatch,
                                                         size_t aStartFrame)
  : AnimationFrameQueue(aBatch, aStartFrame)
...
bool
AnimationFrameRetainedQueue::ResetInternal()
{
...
bool
AnimationFrameRetainedQueue::MarkComplete(const gfx::IntRect& aFirstFrameRefreshArea)
{
  MOZ_ASSERT(!mSizeKnown);
  mSizeKnown = true;
  mPending = 0;
...
void
AnimationFrameRetainedQueue::AdvanceInternal()
{
...
imgFrame*
AnimationFrameRetainedQueue::Get(size_t aFrame, bool aForDisplay)
{
...
bool
AnimationFrameRetainedQueue::IsFirstFrameFinished() const
{
...
void
AnimationFrameRetainedQueue::AddSizeOfExcludingThis(MallocSizeOf aMallocSizeOf,
                                                    size_t& aHeapSizeOut,
...
AnimationFrameDiscardingQueue::AnimationFrameDiscardingQueue(AnimationFrameRetainedQueue& aQueue)
  : AnimationFrameQueue(aQueue)
  , mInsertIndex(aQueue.mFrames.Length())
  , mFirstFrame(Move(aQueue.mFrames[0]))
{
  MOZ_ASSERT(!mSizeKnown);
  ...
  for (size_t i = aQueue.mGetIndex; i < mInsertIndex; ++i) {
    ...
    mDisplay.push_back(Move(aQueue.mFrames[i]));
  }
}
...
AnimationFrameDiscardingQueue::InsertInternal(RefPtr<imgFrame>&& aFrame)
{
  ...
  mDisplay.push_back(Move(aFrame));
  ++mInsertIndex;
...
AnimationFrameDiscardingQueue::AdvanceInternal()
{
  ...
  if (mDisplay.size() + mPending < mBatch) {
    // If we have fewer frames than the batch size, then ask for more. If we
    // do not have any pending, then we know that there is no active decoding.
    mPending += mBatch;
  }
...
AnimationFrameRecyclingQueue::AnimationFrameRecyclingQueue(AnimationFrameRetainedQueue& aQueue)
                                                     ...
                                                     size_t& aExtHandlesOut)
{
  ...
  for (const RecycleEntry& entry : mRecycle) {
    if (entry.mFrame) {
      entry.mFrame->AddSizeOfExcludingThis(aMallocSizeOf, aHeapSizeOut,
                                           aNonHeapSizeOut, aExtHandlesOut);
    }
  }
}

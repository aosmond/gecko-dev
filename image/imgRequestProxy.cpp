...
imgRequestProxy::RemoveFromLoadGroup()
{
  ...
  if (mForceDispatchLoadGroup) {
    ...
    /* We take away the load group from the request temporarily; this prevents
       ...
       BackgroundInLoadGroup from removing and readding. This is safe because
       we know that once we get here, blocking the load group at all is
       unnecessary. */
    mIsInLoadGroup = false;
...
void
imgRequestProxy::BackgroundInLoadGroup()
{
  ...
  if (mIsInLoadGroup && mForceDispatchLoadGroup) {
    LOG_FUNC(gImgLog, "imgRequestProxy::BackgroundInLoadGroup -- dispatch");

    RefPtr<imgRequestProxy> self(this);
    DispatchWithTargetIfAvailable(NS_NewRunnableFunction(
      "imgRequestProxy::BackgroundInLoadGroup",
      [self]() -> void {
        self->BackgroundInLoadGroup();
      }));
    return;
  }

  LOG_FUNC(gImgLog, "imgRequestProxy::BackgroundInLoadGroup");
  nsCOMPtr<imgIRequest> kungFuDeathGrip(this);
  if (mIsInLoadGroup) {
    mLoadGroup->RemoveRequest(this, nullptr, NS_OK);
  }

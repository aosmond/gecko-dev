...
private:
  ...
  Maybe<WriteState> DoWritePixelsToRow(Func aFunc)
  {
    ...
    for (; mCol < mInputSize.width; ++mCol) {
      WriteState writeState = aFunc(rowPtr[mCol]);
      switch (writeState) {
        case WriteState::GOT_PIXEL:
          break;
  ...
  {
    WritePixels<PixelType>([](PixelType& aPixelOut){
      aPixelOut = PixelType(0);
